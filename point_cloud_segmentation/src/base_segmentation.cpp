...
BaseSegmentation::BaseSegmentation() : node("~"), tf_listener(tf_buffer) {
  ...
  x_cutoff_post = node.param<std::vector<float>>("x_cutoff_post",
                                            std::vector<float>{-1000, 1000});
  y_cutoff_post = node.param<std::vector<float>>("y_cutoff_post",
                                            std::vector<float>{-1000, 1000});
  z_cutoff_post = node.param<std::vector<float>>("z_cutoff_post",
                                            std::vector<float>{0.001, 1000});
  segment_scene_srv =
...
void BaseSegmentation::segment_scene_callback(const PointCloudConstPtr &input) {
  if (pub_pc.getNumSubscribers() != 0) {
    ...
    std::vector<PointCloud> pcs;
    segment_scene(received_cloud_ptr, indices, pcs);
    remove_points_not_within_bounds_post(received_cloud_ptr);
...
void BaseSegmentation::remove_points_not_within_bounds_post(PointCloudPtr pc) {
    ...
    #pragma omp parallel for
      for (int i = 0; i < (*pc).size(); i++) {
        if (within_bounds_post(transformed_cloud->points[i])) {
          inliers->indices.push_back(i);
        }
      }
      extract.setInputCloud(pc);
...
bool BaseSegmentation::within_bounds_post(PointT &p) {
  if (p.z > z_cutoff_post[0] and p.z < z_cutoff_post[1])
    if (p.x > x_cutoff_post[0] and p.x < x_cutoff_post[1])
      if (p.y > y_cutoff_post[0] and p.y < y_cutoff_post[1])
        return true;
  return false;
}
