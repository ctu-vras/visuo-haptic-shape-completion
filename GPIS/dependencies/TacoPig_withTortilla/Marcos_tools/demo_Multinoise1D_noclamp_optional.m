...
% GP.CovFn = GP_Remap(GP_CovSum(GP_ExpCov(),GP_SqExpCov()), [1 1 2 2]); % learn a sqexp with length scale = variance
  ...
  %GP.CovFn = GP_Remap(GP_SqExpCov(), [1 1]);  %Isotropic Squared Exponential
...
%       GP_ClampNoise(noisefn, params_index, params_values); 
   ...
   % GP.NoiseFn = GP_StatNoise();
   % GP_ClampNoise( GP.NoiseFn, [1], 10);     
   
%GP.NoiseFn = GP_ClampNoise(GP_MultiNoise([10 10]),[1], [2]); 
%GP.NoiseFn = GP_ClampNoise(GP_MultiNoise([20]),[1],[0.5]);
%GP.NoiseFn = GP_ClampNoise(GP_MultiNoise([ones(1,length(X))]), [1:length(X)], [0.5*ones(1,length(X))]);  % learning point by point
%GPLr.NoiseFn = GP_ClampNoise(GP_MultiNoise([nr+ngpl]) ,[1], [0.001]); 
GP.NoiseFn = GP_MultiNoise([ones(1,length(X))]); 
%GP.NoiseFn = GP_MultiNoise([7 8 5]); 
