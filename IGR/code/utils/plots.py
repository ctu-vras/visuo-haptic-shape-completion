...
def plot_surface(decoder,path,epoch, shapename, rep, resolution,mc_value,is_uniform_grid,verbose,save_html,save_ply,overwrite, points=None, with_points=False, latent=None, connected=False):
    filename = '{0}/igr_{1}_{2}'.format(path, epoch, shapename)+"_"+str(rep)
    if (not os.path.exists(filename) or overwrite):
        ...
        surface, mesh = get_surface_trace(points,decoder,latent,resolution,mc_value,is_uniform_grid,verbose,save_ply, connected)
        ...
        return surface['mesh_export'], mesh
...
def get_surface_trace(points,decoder,latent,resolution,mc_value,is_uniform,verbose,save_ply, connected=False):
    ...
    return {"mesh_trace":trace, "mesh_export":meshexport},\
           z.reshape(grid['xyz'][1].shape[0], grid['xyz'][0].shape[0], grid['xyz'][2].shape[0]).transpose([1, 0, 2])
...
def get_grid(points,resolution):
    eps = 0.1
    input_min = torch.min(points, dim=0)[0].squeeze().cpu().detach().numpy()
    input_max = torch.max(points, dim=0)[0].squeeze().cpu().detach().numpy()
