...
class ShapeCompletionDataset(data.Dataset):
    ...
    def get_info(self, index):
        ...
        for rr in [1, 2, 3, 4, 5, 6]:
            if shape_name.split("_")[rr].split("-")[0] == "2021":
                obj_name = "_".join(shape_name.split("_")[:rr])
                break
        return "npy", "_".join(obj_name.split("_")[:2]), tag[:tag.find('.npy')]
...
def get_instance_filenames(base_dir, split, ext='', format='npy'):
    ...
    for dataset in split:
        for shape in split[dataset]:
            for rr in [1, 2, 3, 4, 5, 6]:
                if shape.split("_")[rr].split("-")[0] == "2021":
                    obj_name = "_".join(shape.split("_")[:rr])
                    break
            instance_filename = os.path.join(base_dir, shape, obj_name + "{0}.{1}".format(ext, format))
